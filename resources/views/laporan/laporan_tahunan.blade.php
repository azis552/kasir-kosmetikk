    ...
    <div class="container">
        <div class="card p-2">
            <div class="card-header">
                <h5>Laporan Tahunan {{ $tahun }}</h5>
                <form class="d-flex align-items-center mt-3" action="{{ route('laporan.laporan_tahunan') }}" method="GET">

                    <input type="year" name="tahun" id="tahun" class="form-control mr-2"
                        value="{{ request('tahun') ?? $tahun ?? now()->format('Y') }}">

                    <button type="submit" class="btn btn-primary mr-2">Cari</button>

                    <a id="cetak"
                        href="{{ route('laporan.laporan_tahunan', ['tahun' => (request('tahun') ?? $tahun), 'pdf' => true]) }}"
                        class="btn btn-success text-nowrap px-4 {{ (request('tahun') ?? $tahun) ? '' : 'disabled-link' }}">
                        Cetak PDF
                    </a>
                </form>
            </div>
